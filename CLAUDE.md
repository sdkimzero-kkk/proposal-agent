...
**v3.7 업데이트 (1000장 확장)**: Phase-Split 생성 패턴 + 대형 RFP 청크 분석 + PPTX 병합 유틸
...
**STEP 1 (대형 RFP 처리)**: RFP가 100페이지 이상이면 청크 분할 리더 사용
```python
from src.utils.chunked_pdf_reader import read_large_rfp
data = read_large_rfp("제안요청서/테스트 XX/rfp.pdf")
# data = {"raw_text": ..., "tables": ..., "page_count": ..., "chunks": [...]}
```
→ `rfp_analyzer.py`가 대용량 raw_text를 자동으로 청크 분석하여 핵심 정보 추출

**STEP 3-A: 슬라이드 계획 수립** (`proposal_outline.md` 작성)

규모별 Phase 장수 기준:

| 규모 | 총 슬라이드 | 생성 방식 |
|------|----------|---------|
| 소형 (40~80장) | 1개 통합 스크립트 | 기존 방식 (generate_제안서.py) |
| 중형 (80~200장) | Phase별 분할 | **Phase-Split 패턴** 사용 |
| 대형 (200~1000장) | Phase별 분할 + 청크 RFP | **Phase-Split 패턴** 사용 |

**중형 이상이면 반드시 Phase-Split 패턴 사용** (아래 STEP 3-B~C):

```
# proposal_outline.md 예시 (Phase별 시작 페이지 계획표)
| Phase | 이름 | 장수 | 시작 pg |
|-------|------|------|--------|
| cover | 표지+목차 | 3 | (번호 없음) |
| 0 | HOOK | 8 | 4 |
...
**STEP 3-B: Phase별 스크립트 생성**

Phase-Split 출력 구조:
```
output/테스트 XX/
├── proposal_outline.md       ← 슬라이드 계획표
├── generate_cover_toc.py     ← 표지 + 목차
├── generate_phase0.py        ← HOOK
├── generate_phase1.py        ← SUMMARY
├── generate_phase2.py        ← INSIGHT
├── generate_phase3.py        ← CONCEPT & STRATEGY
├── generate_phase4.py        ← ACTION PLAN (가장 길 수 있음)
├── generate_phase5.py        ← MANAGEMENT
├── generate_phase6.py        ← WHY US
├── generate_phase7.py        ← INVESTMENT
├── merge_pptx.py             ← 자동 병합 (아래 참조)
└── pptx/                     ← 중간 결과물 폴더
    ├── cover_toc.pptx
    ├── phase0.pptx  ...  phase7.pptx
```

각 Phase 스크립트 필수 패턴:
```python
#!/usr/bin/env python3
"""Phase N: [Phase명] — [프로젝트명]"""
import sys, os
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
sys.path.insert(0, PROJECT_ROOT)
from src.generators.slide_kit import *

os.makedirs(os.path.join(os.path.dirname(__file__), "pptx"), exist_ok=True)

prs = new_presentation()
WIN = {"key1": "Win Theme 1", "key2": "Win Theme 2", "key3": "Win Theme 3"}
PG_START = [시작_페이지_번호]  # proposal_outline.md 기준

# 섹션 구분자
slide_section_divider(prs, "0N", "Phase명", "부제", "스토리", "key1", WIN)

# 콘텐츠 슬라이드들 (pg=PG_START + 로컬카운터)
s = new_slide(prs)
bg(s, C["white"])
TB(s, "Action Title", pg=PG_START)
# ... 콘텐츠 ...
PN(s, PG_START)

save_pptx(prs, os.path.join(os.path.dirname(__file__), "pptx/phaseN.pptx"))
print(f"Phase N 저장 완료: {len(prs.slides)}장")
```

**STEP 3-C: merge_pptx.py 생성**
```python
#!/usr/bin/env python3
"""Phase별 PPTX 병합"""
import sys, os
...
FILES = [
    os.path.join(BASE, "pptx/cover_toc.pptx"),
    os.path.join(BASE, "pptx/phase0.pptx"),
    os.path.join(BASE, "pptx/phase1.pptx"),
    os.path.join(BASE, "pptx/phase2.pptx"),
    os.path.join(BASE, "pptx/phase3.pptx"),
    os.path.join(BASE, "pptx/phase4.pptx"),
    os.path.join(BASE, "pptx/phase5.pptx"),
    os.path.join(BASE, "pptx/phase6.pptx"),
    os.path.join(BASE, "pptx/phase7.pptx"),
]
OUTPUT = os.path.join(BASE, "제안서_완성.pptx")

total = merge_pptx_files(FILES, OUTPUT)
print(f"병합 완료: 총 {total}장 → {OUTPUT}")
```
...
소형 (단일 스크립트):
```bash
python generate_제안서.py
```

중형/대형 (Phase-Split):
```bash
# Phase별 순서 실행
python generate_phase0.py && python generate_phase1.py && \
python generate_phase2.py && python generate_phase3.py && \
python generate_phase4.py && python generate_phase5.py && \
python generate_phase6.py && python generate_phase7.py && \
python generate_cover_toc.py

# 병합
python merge_pptx.py
```

- 오류 발생 시 해당 Phase 스크립트만 수정 후 재실행
- 최종 파일: `output/테스트 XX/제안서_완성.pptx`
