...
## RFP 분석 결과 (Phase 0 관련 항목)
{self._build_rfp_context(0, rfp_analysis)}
...
## RFP 분석 결과 (Phase {phase_num} 관련 항목)
{self._build_rfp_context(phase_num, rfp_analysis)}
...
"""
    ...
    # ── Phase별 컨텍스트 슬라이싱 ─────────────────────────────────────
    #
    # 각 Phase에 필요한 RFPAnalysis 필드만 추출하여 토큰을 절약합니다.
    # Phase 0(HOOK)부터 Phase 7(INVESTMENT)까지 다른 필드 조합을 사용합니다.
    #
    # 예상 절감: 전체 전달(~10,000자) → Phase별 전달(~2,000~4,000자) = 40~70%

    # Phase별 필요 필드 정의
    _PHASE_RFP_FIELDS: Dict[int, List[str]] = {
        0: [  # HOOK: 슬로건/임팩트 생성에 필요한 핵심만
            "project_name", "client_name", "project_overview",
            "pain_points", "winning_strategy", "win_theme_candidates",
        ],
        1: [  # SUMMARY: Executive Summary — 전체 그림 필요
            "project_name", "client_name", "project_overview",
            "key_requirements", "evaluation_criteria",
            "winning_strategy", "win_theme_candidates",
            "pain_points", "differentiation_points",
        ],
        2: [  # INSIGHT: 시장/문제 분석
            "project_overview", "pain_points", "hidden_needs",
            "potential_risks", "competitive_landscape",
            "key_success_factors",
        ],
        3: [  # CONCEPT: 전략/차별화
            "winning_strategy", "win_theme_candidates",
            "differentiation_points", "pain_points",
            "evaluation_strategy", "key_requirements",
            "competitive_landscape",
        ],
        4: [  # ACTION: 상세 실행 — 요구사항/산출물/일정 중심
            "key_requirements", "technical_requirements",
            "functional_requirements", "deliverables",
            "timeline", "evaluation_criteria", "evaluation_strategy",
        ],
        5: [  # MANAGEMENT: 운영/품질
            "deliverables", "timeline",
            "potential_risks", "key_success_factors",
        ],
        6: [  # WHY US: 역량/실적
            "evaluation_criteria", "competitive_landscape",
            "differentiation_points", "winning_strategy",
            "win_theme_candidates", "evaluation_strategy",
        ],
        7: [  # INVESTMENT: 예산/ROI
            "budget", "timeline", "deliverables",
            "key_success_factors", "differentiation_points",
        ],
    }

    def _build_rfp_context(self, phase_num: int, rfp_analysis: RFPAnalysis) -> str:
        """Phase에 필요한 RFPAnalysis 필드만 추출하여 JSON 문자열로 반환.

        전체 model_dump() 대신 Phase별 관련 필드만 선택하여
        토큰 사용량을 40~70% 절감합니다.
        """
        fields = self._PHASE_RFP_FIELDS.get(phase_num, list(self._PHASE_RFP_FIELDS[1]))
        full = rfp_analysis.model_dump()
        sliced = {k: full[k] for k in fields if k in full}

        # 리스트 항목 수 제한 (Phase 4는 요구사항이 많으므로 20개, 나머지 10개)
        list_limit = 20 if phase_num == 4 else 10
        for k, v in sliced.items():
            if isinstance(v, list) and len(v) > list_limit:
                sliced[k] = v[:list_limit]

        result = json.dumps(sliced, ensure_ascii=False, indent=2)
        # 최대 6000자로 제한 (Phase 4는 8000자)
        max_chars = 8000 if phase_num == 4 else 6000
